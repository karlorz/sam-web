...
// Import sam-web library from local source
import {
  SAMClient,
  MODEL_CONFIGS,
  type SegmentResult,
...
export default function SAMDemo() {
  ...
  useEffect(() => {
    ...
    // Initialize with bundled worker (for Storybook)
    import('../core/worker.ts?worker').then((WorkerModule) => {
      const InlineWorker = WorkerModule.default;
      const worker = new InlineWorker();
      return client.initializeWithWorker(worker);
    })
      .then(async () => {
